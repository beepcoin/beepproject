...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BeepCoinTEST2\r");
            Send(hSocket, "WHO #BeepCoinTEST2\r");
        } else {
            // randomly join #BeepCoin00-#BeepCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BeepCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BeepCoin%02d\r", channel_number).c_str());
        }
