BeepCoin-qt: Qt4 GUI for BeepCoin
===============================
...
Alternatively, install Qt Creator and open the `BeepCoin-qt.pro` file.

An executable named `BeepCoin-qt` will be built.
...
A warning for people using the *static binary* version of BeepCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BeepCoin is linked against libdb5.3.
...
installed causes BeepCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
